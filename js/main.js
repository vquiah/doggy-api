...
fetch(breed_url)
  ...
  .then(data=>{
    ...
    for (let i =0; i < breedsArray.length; i++){
      const options = document.createElement('option')// <option></option>
      options.value = breedsArray[i] // <option value = 'breed'>
       options.innerText = breedsArray[i] // <option value = 'breed'> breed</option>
      select.appendChild(options) //creatte another tag within the parent tag,,,,add current option tag to the select box list of option
    }
  })
select.addEventListener('change', event =>{
  ...
  getDoggoImg(url)
  doggoInffo.assignMF();
  doggoInffo.assignAge();
  doggoInffo.assignLikes();
  doggoInffo.assignDislikes();
  doggoInffo.assignFunFact();
})
...
const getDoggoImg = url =>{
  fetch(url) //going to the api url above
    ...
    .then(data =>{
      console.log(data.message); //extract message from from json and attach to img tag as new source
...
assignMF(){
  ...
  document.querySelector('#MF').innerText= `S: ${this.MF}`

},
  assignName(array){
    this.rname = array[Math.floor(Math.random()*array.length)]
    document.querySelector('.name').innerText= `${this.rname}`
  },
  assignAge() {
